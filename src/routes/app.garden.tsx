...
import { PageHeader, SoftCard, ProgressBar, NumiAvatar, SectionTitle, EmptyState } from "@/components/numind/ui-kit";
...
export const Route = createFileRoute("/app/garden")({
  head: () => ({
    meta: [
      ...
      { name: "description", content: "Your Wellness Garden grows from seed to sanctuary as you show up for yourself." },
      { property: "og:title", content: "My Garden — NuMind" },
      { property: "og:description", content: "Your Wellness Garden grows from seed to sanctuary as you show up for yourself." },
    ],
  }),
...
function GardenPage() {
  ...
  return (
    <div className="mx-auto max-w-5xl">
      <PageHeader emoji="🌱" title="My Garden" subtitle="Every activity you finish helps something here grow." />
      ...
      <div className="relative overflow-hidden rounded-4xl bg-garden p-6 sm:p-10">
        ...
        <div className="grid place-items-center gap-3 py-8 text-center">
          <span className="animate-float text-8xl" aria-hidden>{gardenStage.emoji}</span>
      ...
      <div className="mt-6 flex gap-2">
        {(["garden", "items"] as const).map((t) => (
          <button
            ...
            onClick={() => setTab(t)}
            aria-pressed={tab === t}
            className={cn("focus-ring rounded-full px-4 py-2 text-sm font-medium", tab === t ? "bg-brand font-bold text-navy" : "bg-muted")}
          >
      ...
      {tab === "garden" ? (
        <SoftCard className="mt-4">
          ...
          <ul className="grid gap-2 text-sm">
            <li className="rounded-2xl bg-muted/60 px-4 py-3">🌞 Daily Reset · +10 Garden XP · today</li>
            <li className="rounded-2xl bg-muted/60 px-4 py-3">🦋 Butterfly Flock unlocked · yesterday</li>
            <li className="rounded-2xl bg-muted/60 px-4 py-3">🧘 Mind Gym · +10 Garden XP · yesterday</li>
          </ul>
        </SoftCard>
      ) : (
          ...
          {ITEMS.map((i) => (
            <li key={i.name}>
              <div className={cn("card-soft p-4 text-center", !i.owned && "opacity-60")}>
                <p className="text-3xl" aria-hidden>{i.owned ? i.emoji : "🔒"}</p>
                <p className="mt-2 text-sm font-semibold">{i.name}</p>
                <p className="text-xs text-muted-foreground">{i.owned ? "Placed" : "Unlock in Rewards"}</p>
              </div>
            </li>
          ))}
        </ul>
      ...
      {gardenXp === 0 ? (
        <div className="mt-6">
          <EmptyState emoji="🌱" title="Your garden is waiting" message="Your garden is waiting for its first seed. 🌱" />
        </div>
      ) : null}
    </div>
  );
